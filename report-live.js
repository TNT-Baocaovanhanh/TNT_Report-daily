...
(function () {
  ...
  const GAS_URL_KEY = 'tnt_gas_url_seen';
  ...
  function getReportSiteHint() {
    const o = (typeof window !== 'undefined' && window.location && window.location.origin)
      ? window.location.origin : '';
    if (o.indexOf('tnt-baocaovanhanh.github.io') >= 0) {
      return 'tnt-baocaovanhanh.github.io/TNT_Report-daily';
    }
    if (o.indexOf('anhquanvp.github.io') >= 0) {
      return 'anhquanvp.github.io/Reportdaily';
    }
    return 'anhquanvp.github.io hoặc tnt-baocaovanhanh.github.io';
  }
  ...
  function handleGoogleCredential(response) {
    ...
    // Gọi GAS qua JSONP (script tag) để KHÔNG rời khỏi trang báo cáo (GitHub Pages).
    // Trước đây dùng form POST → trình duyệt nhảy hẳn sang script.google.com.
    gasJsonp(cfg, { action: 'auth_jwt', credential: response.credential }, 30000)
      .then(function (json) {
        if (json && (json.ok || json.auth) && json.auth_token) {
          saveSession(json.auth_token, json.email);
          unlockReport(json.email);
          setStatus('✅ Đã đăng nhập: ' + json.email, 'ok');
          setTimeout(refreshData, 300);
        } else {
          clearSession();
          lockReport();
          showAuthError((json && json.error) || 'Đăng nhập thất bại — thử lại.');
          setStatus('❌ Đăng nhập thất bại', 'err');
        }
      })
      .catch(function (err) {
        clearSession();
        ...
        showAuthError('Lỗi kết nối GAS khi đăng nhập: ' + m);
        setStatus('❌ ' + m, 'err');
      });
  }
  ...
  function getConfig() {
    ...
    return {
      GAS_URL: builtin.GAS_URL || String(stored.GAS_URL || '').trim(),
      GOOGLE_CLIENT_ID: builtin.GOOGLE_CLIENT_ID || String(stored.GOOGLE_CLIENT_ID || '').trim(),
      TG_BOT_TOKEN: String(stored.TG_BOT_TOKEN || builtin.TG_BOT_TOKEN || '').trim(),
      TG_CHAT_ID: String(stored.TG_CHAT_ID || builtin.TG_CHAT_ID || '').trim(),
    };
  }

  /** Chỉ lưu Telegram + GAS URL khi admin chưa cấu hình sẵn trong TNT_CONFIG */
  function persistUserConfig(partial) {
    const builtin = getBuiltinConfig();
    const next = getStoredConfig();
    if (partial.TG_BOT_TOKEN !== undefined) next.TG_BOT_TOKEN = partial.TG_BOT_TOKEN;
    if (partial.TG_CHAT_ID !== undefined) next.TG_CHAT_ID = partial.TG_CHAT_ID;
    if (!builtin.GAS_URL && partial.GAS_URL !== undefined) next.GAS_URL = partial.GAS_URL;
  ...
  function getApiConfig() {
    const cfg = getConfig();
    if (resolvedGasUrl) {
      return Object.assign({}, cfg, { GAS_URL: resolvedGasUrl });
    }
    return cfg;
  }

  function gasDeploymentId(url) {
    const m = String(url || '').match(/\/macros\/s\/([^/]+)/);
    return m ? m[1] : '';
  }

  /** Workspace GHN dùng /a/ghn.vn/ — đồng bộ URL thật từ GAS ping */
  async function ensureCanonicalGasUrl() {
    const cfg = getConfig();
    if (!cfg.GAS_URL) return getApiConfig();
    try {
      const json = await gasJsonp(cfg, { action: 'ping' }, 25000);
      if (json && json.oauthRedirectUri) {
        const canonical = String(json.oauthRedirectUri).replace(/\/$/, '');
        const current = String(cfg.GAS_URL).replace(/\/$/, '');
        if (canonical && canonical !== current) {
          const idA = gasDeploymentId(canonical);
          const idB = gasDeploymentId(current);
          if (!idB || idA === idB) {
            resolvedGasUrl = canonical;
          }
        }
      }
      if (json && json.clientSecretStatus === 'wrong_format') {
        console.warn('[TNT] GOOGLE_CLIENT_SECRET sai định dạng — dùng Google Sign-In, không cần secret');
      }
    } catch (e) {
      console.warn('[TNT] ping GAS:', e.message);
    }
    return getApiConfig();
  }

  /** Mỗi trình duyệt/máy có localStorage riêng — xóa token/URL cũ khi admin đổi deploy */
  function migrateClientStorage() {
    const builtin = getBuiltinConfig();

    if (builtin.GAS_URL) {
      const prevGas = localStorage.getItem(GAS_URL_KEY);
      if (prevGas && prevGas !== builtin.GAS_URL) {
        clearSession();
      }
      localStorage.setItem(GAS_URL_KEY, builtin.GAS_URL);

      const stored = getStoredConfig();
      if (stored.GAS_URL && stored.GAS_URL !== builtin.GAS_URL) {
        delete stored.GAS_URL;
        localStorage.setItem(STORAGE_KEY, JSON.stringify(stored));
      }
    }

    const prevVer = localStorage.getItem('tnt_app_version');
    if (prevVer && prevVer !== APP_VERSION) {
      clearSession();
    }
    localStorage.setItem('tnt_app_version', APP_VERSION);
  }
  ...
  function loginViaGas() {
    ...
    window.location.href = buildGasUrl(cfg, { action: 'auth', cb: cb });
  }
  ...
  function login() {
    ...
    if (!cfg.GAS_URL) {
      const setup = document.getElementById('auth-gas-setup');
      if (setup) setup.style.display = 'block';
      showAuthError('<strong>Chưa có URL API.</strong> Liên hệ admin cấu hình GAS URL.');
      return;
    }
    if (!getGoogleClientId()) {
      showAuthError('<strong>Thiếu GOOGLE_CLIENT_ID</strong> trong TNT_CONFIG.');
      return;
    }
    function tryPrompt() {
      if (!window.google || !window.google.accounts || !window.google.accounts.id) {
      ...
      window.google.accounts.id.prompt(function (notification) {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          showAuthError(
            'Google Sign-In không hiển thị. Mở báo cáo qua GitHub Pages '
            + '(<code>' + getReportSiteHint() + '</code>) và thử lại.'
          );
        }
      });
    }
  ...
  async function ensureAuth() {
    ...
    if (fromHash && fromHash.deny) {
      ...
      clearSession();
      lockReport();
      if (fromHash.deny === 'use_gsi') {
        showAuthError('Bấm nút <em>Đăng nhập Google</em> ngay trên trang báo cáo này (không mở link GAS trực tiếp).');
      } else if (fromHash.deny === 'bad_callback') {
        showAuthError(
          'URL trang báo cáo không hợp lệ khi đăng nhập. Mở đúng link GitHub Pages '
          + '(<code>' + getReportSiteHint() + '</code>) rồi đăng nhập lại.'
        );
      } else if (fromHash.deny === 'oauth_config' || fromHash.deny === 'no_email') {
        showAuthError(
          '<strong>Đăng nhập OAuth thất bại.</strong><br>'
          + 'Admin cần: (1) thêm <code>GOOGLE_CLIENT_SECRET</code> vào Script properties, '
          + '(2) thêm Redirect URI trong GCP khớp URL trên trang cấu hình GAS, '
          + '(3) deploy <strong>New version</strong>.'
        );
      } else {
  ...
  async function refreshData() {
    ...
    if (!getAuthToken()) {
      lockReport();
      showAuthError('Chưa đăng nhập — bấm <em>Đăng nhập Google</em> để xem và làm mới dữ liệu.');
      setStatus('❌ Thiếu auth_token — đăng nhập lại', 'err');
      return;
    }
  ...
  function openSettings() {
    ...
    if (gasInp) {
      gasInp.value = cfg.GAS_URL || '';
      gasInp.readOnly = !!builtin.GAS_URL;
      gasInp.title = builtin.GAS_URL ? 'URL do admin cấu hình trong TNT_CONFIG' : '';
    }
  ...
  function saveSettings() {
    ...
    if (getConfig().GAS_URL && getAuthToken()) refreshData();
  }
  ...
  function setupAuthUI() {
    ...
    if (gsiWrap && getGoogleClientId()) {
      gsiWrap.style.display = 'flex';
      initGoogleSignIn(gsiWrap);
    } else if (gsiWrap) {
      gsiWrap.style.display = 'none';
    }
    ...
    if (loginBtn) {
      ...
      setTimeout(function () {
        if (gsiWrap && (gsiWrap.querySelector('iframe') || gsiWrap.children.length > 1)) {
